...
output_pptx = rf"{derivatives_folder}\FF_single_subject_glm_smooth_6_unthresholded_20250709.pptx"
...
max_col_num = 3
num_rows = int(np.ceil(len(subject_labels) / max_col_num))
...
for c, contrast_name in enumerate(contrast_names):
    ...
    for s, subject_label in enumerate(subject_labels):
        subject_folder = Path(derivatives_folder, subject_label, 'glm_ff')
        row = s // max_col_num
        col = s % max_col_num

        print(subject_label)

        image = list(subject_folder.glob(f'ff*{subject_label}*{contrast_name}*UNTHRESHOLDED*.png'))[0]
