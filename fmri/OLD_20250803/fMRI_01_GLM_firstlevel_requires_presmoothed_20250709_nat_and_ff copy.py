...
#1. #select behavioral task used either 'nat' (derivatives_folderexperiment) or 'fhof' (localizer) 
task = "nat" 
# task = "ff"
#2. select glm folder
nat_glm_folder = "glm"
ff_glm_folder  = "glm_ff"
#3. select subjects to process by adding them to subject_labels list:
nat_labels = ['sub-01', 'sub-02', 'sub-03', 'sub-04', 'sub-05', 'sub-06', 'sub-07', 'sub-08', 'sub-09'] 
ff_labels = ['sub-01', 'sub-02', 'sub-03', 'sub-04', 'sub-05', 'sub-07', 'sub-08', 'sub-09'] 

nat_labels = ['sub-09']
#4. select alphas for effects:
nat_main_effects            = ["Allo_Main", "Ego_Main", "Color_Main", "Instructions"]
nat_main_effects_pval       = 0.01
nat_contrast_effects        = ["Ego-Color", "Allo-Color", "Allo-Ego"]
nat_contrast_effects_pval   = 0.1

ff_main_effects            = ["Color Flags", "Ego Flags",   "Allo Flags",  "Color Faces"	" Ego Faces",   "Allo Faces"]
ff_main_effects_pval       = 0.05
ff_contrast_effects        = ["Ego-Color_Flags", "Allo-Color_Flags", "Allo-Ego_Flags", "Ego-Color_Faces", "Allo-Color_Faces", "Allo-Ego_Faces"]
ff_contrast_effects_pval   = 0.05


if task == "nat": 
    subject_labels = nat_labels 
    main_effects  = nat_main_effects  
    main_effects_pval = nat_main_effects_pval 
    contrast_effects  = nat_contrast_effects  
    contrast_effects_pval = nat_contrast_effects_pval 
    glm_folder = nat_glm_folder

elif task == "ff":
    subject_labels = ff_labels 
    main_effects  = ff_main_effects  
    main_effects_pval = ff_main_effects_pval 
    contrast_effects  = ff_contrast_effects  
    contrast_effects_pval = ff_contrast_effects_pval 
    glm_folder = ff_glm_folder

#compute zscore thresholds for alphas
...
for subject_label in subject_labels:
    ...
    glm_path = Path(subjectfolder, glm_folder)
    if not glm_path.exists():
        os.makedirs(glm_path), print(f"made {glm_path}")
    ...
    for r, run_BOLD_file in enumerate(run_BOLD_files_raw): #For each BOLD file (run) get run_id (number)
        ...
        run_events_file = Path(subjectfolder,'events', f'{subject_label}_{run_id}_events.tsv') #Get events files
...
for subject_label in subject_labels:
        ...
        effect_dict = {
          ...
          effect_dict = {
            ...
            #   "Allo Faces",                   effect_contrasts["Allo Faces"],
              "Ego-Color_Flags":              effect_contrasts["Ego Flags"]  - effect_contrasts["Color Flags"],              
              "Allo-Color_Flags":             effect_contrasts["Allo Flags"] - effect_contrasts["Color Flags"],
              "Allo-Ego_Flags":               effect_contrasts["Allo Flags"] - effect_contrasts["Ego Flags"],
              "Ego-Color_Faces":              effect_contrasts["Ego Faces"]  - effect_contrasts["Color Faces"],              
              "Allo-Color_Faces":             effect_contrasts["Allo Faces"] - effect_contrasts["Color Faces"],
              "Allo-Ego_Faces":               effect_contrasts["Allo Faces"] - effect_contrasts["Ego Faces"],
            }
    ...
    for effect_id, effect_val in effect_dict.items():
        ...
        z_map.to_filename(output_file) #save niimg to file 
    ...
    for i, effect_id in enumerate(z_maps_to_plot): #for each key in z_maps_to_plot
        ...
        output_file     =  rf'{subjectfolder_glm}/{task}_{subject_label}_glm_{effect_id}_smooth_{smoothing_parameter}_uncorrected_p={pval}_surface_AMENDED.png'
        plotting.plot_img_on_surf(
            ...
            darkness        = 1.0,
            title           = f'{task}_{subject_label}_glm_{effect_id}_uncorrected_p={pval}',
            output_file     = output_file,
        )
# %%
