...
nat_contrast_effects_pval   = 0.05

ff_main_effects            = ["Color_Flags", "Ego_Flags",   "Allo_Flags",  "Color_Faces",	" Ego_Faces",   "Allo_Faces"]
...
contrasts_to_model = contrast_effects + main_effects
...
for contrast in contrasts_to_model:
    ...
    for subject_label in subject_labels: #load images
        ...
        #Get contrast file
        contrast_file = list(Path(subjectfolder, glm_folder).glob(f'*glm*{contrast}*z_map.nii*'))[0]
    ...
    # 2) Concatenate into a single 4-D image
    imgs_4d = image.concat_imgs(contrast_img)          # shape: (x, y, z, n_subjs)

    # 3) Compute voxel-wise standard deviation across subjects
    sd_img = image.math_img("np.std(img, axis=3)", img=imgs_4d)

    # 4) Save for later
    sd_img.to_filename(f"{derivatives_folder}/{task}_{contrast}_sd_between_subjects.nii.gz")


       
    # PLOT 3D SURFACES PIAL STATIC .PNG
    output_file     =  rf'{derivatives_folder}/{task}_{contrast}_sd_between_subjects_surface.png'
    plotting.plot_img_on_surf(
        ...
        threshold       = None,
        vmin=-5,       # lower bound of colorbar
        vmax=5,        # upper bound of colorbar
        colorbar        = True,
        darkness        = 1.0,
        title           = f'{task}__sd_between_subjects_{contrast}_20250708',
