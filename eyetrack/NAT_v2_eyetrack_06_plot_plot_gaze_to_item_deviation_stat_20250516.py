    ...
    if s==1:
        ...
        for r, run in enumerate(runs):  
            # if r==0:
                ...
                press_indices   = np.array(data.index[data['Events'] == 'KeyPress'])
                ...
                window        = np.arange(timewindow_start_index,
                          timewindow_end_index)     # (n_window,)   e.g. [-3, -2, -1, 0, 1, 2]

                ioi = np.array((press_indices[:, None] + window).ravel()) #create list of press indices
                flag_names_stretched = np.repeat(flag_names, window.size) #repeat flag names
                data.loc[ioi, 'flag_name'] = flag_names_stretched



                press_indices   = np.array(all_data.index[all_data['Events'] == 'KeyPress'])
                flag_config = all_data['flag_config'].iloc[press_indices]
                condition = all_data['condition_names1'].iloc[press_indices]
                flag_name = all_data['flag_name']
                if "key" in condition: # if key
                    if "OKE" in flag_config:
                        x = get_value(all_data, r, f'{flag_name}_LeftPosX$')
                        y = get_value(all_data, r, f'{flag_name}_LeftPosY$')
                    elif "OEK" in flag_config:
                        x = get_value(all_data, r, f'{flag_name}_RightPosX$')
                        y = get_value(all_data, r, f'{flag_name}_RightPosY$')
                ...
                all_presses_data.append(data.iloc[ioi])


all_data = pd.concat(all_presses_data)
...
#                     all_data = data.iloc[timewindow_start_ind:timewindow_end_ind]
